...
#' @examples
#' \dontrun{
#' plotDemandCurve(y_axis_range = seq(0, 10000, by = 100), scenario = "ssp2")
#' }
#'
...
plotDemandCurve <- function(y_axis_range, region = "GLO", scenario, lpjml, selectyears, iniyear, climatetype, EFRmethod, accessibilityrule, rankmethod, yieldcalib, allocationrule, thresholdtype, irrigationsystem, cropmix, potential_wat = TRUE, com_ag, multicropping) {
  ...
  # area_potirrig_curr <- reportYieldgainArea(region = region, GT_range = y_axis_range, scenario = scenario, lpjml = lpjml,
  ...
  # area_potirrig_pot  <- reportYieldgainArea(region = region, GT_range = y_axis_range, scenario = scenario, lpjml = lpjml,
  #                                           selectyears = selectyears, climatetype = climatetype, EFRmethod = EFRmethod,
