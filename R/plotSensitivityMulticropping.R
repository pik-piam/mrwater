...
#' @param efrMethod        EFR method: VMF:fair, Smakthin:good
#'
...
plotSensitivityMulticropping <- function(x_axis_range, region = "GLO", output, scenario, lpjml, iniyear, selectyears, climatetype, efrMethod, accessibilityrule, rankmethod, yieldcalib, allocationrule, thresholdtype, irrigationsystem, landScen, cropmix, potential_wat = TRUE, com_ag) {

  MCTrue  <- reportEconOfIrrig(GT_range = x_axis_range, region = region, output = output, iniyear = iniyear, scenario = scenario, lpjml = lpjml, selectyears = selectyears, climatetype = climatetype,
    ...
    cropmix = cropmix, potential_wat = TRUE, com_ag = com_ag, multicropping = TRUE)$data
  MCTrue$Accessibility <- rep("MCTrue", length(MCTrue$GT))

  MCFalse  <- reportEconOfIrrig(GT_range = x_axis_range, region = region, output = output, iniyear = iniyear, scenario = scenario, lpjml = lpjml, selectyears = selectyears, climatetype = climatetype,
    efrMethod = efrMethod, accessibilityrule = accessibilityrule, rankmethod = rankmethod, yieldcalib = yieldcalib,
    allocationrule = allocationrule, thresholdtype = thresholdtype, irrigationsystem = irrigationsystem, landScen = landScen,
    cropmix = cropmix, potential_wat = TRUE, com_ag = com_ag, multicropping = FALSE)$data
  MCFalse$Accessibility <- rep("MCFalse", length(MCFalse$GT))
  ...
  df <- rbind.data.frame(MCFalse, MCTrue)
