...
calcRiverSurplusDischargeAllocation <- function(lpjml, climatetype,
  ...
  fullIrrigReqWW <- pmax(collapseNames(fullIrrigReq[, , "withdrawal"]), 0)
  fullIrrigReqWC <- pmax(collapseNames(fullIrrigReq[, , "consumption"]), 0)
  ...
  fullirrigFRAC <- new.magpie(cells_and_regions = getCells(discharge),
                              years = getYears(discharge),
                              names = getNames(yieldgain),
                              fill = 0,
  ...
  fullirrigFRAC  <- as.array(.transformObject(fullirrigFRAC))
  fullIrrigReqWW <- as.array(.transformObject(fullIrrigReqWW))
  fullIrrigReqWC <- as.array(.transformObject(fullIrrigReqWC))
  yieldgain      <- as.array(.transformObject(yieldgain))
  iwpWW          <- as.array(.transformObject(0))
  iwpWC          <- as.array(.transformObject(0))
  ...
  inLIST    <- list(yieldgain = yieldgain,
                    fullIrrigReqWW = fullIrrigReqWW,
                    fullIrrigReqWC = fullIrrigReqWC,
  ...
  for (y in selectyears) {
    ...
    inoutLIST <- toolDischargeAllocation(y = y, rs = rs,
                                     ...
                                     glocellrank = glocellrank, allocationrule = allocationrule)
  }
  ...
  if (output == "discharge") {
    ...
    frac        <- as.magpie(inoutLIST$fullirrigFRAC, spatial = 1)
    out         <- frac * fullIrrigReq
