...
toolRiverUpDownBalanceSINGLE <- function(inLIST, inoutLIST) {
  ...
  if ((discharge[1] - prevWW) > 0) {
    ...
    discharge <- discharge - prevWC - currRequestWClocal[1]
  } else {
    ...
    if (length(currRequestWClocal) > 1) {
      ...
      if (upstreamWC > (prevWW - discharge[1])) {
        ...
        frac <- (1 - (prevWW - discharge[1]) / upstreamWC)
        ...
        discharge <- discharge + upstreamWC - prevWC
      }
    } else {
      # Update discharge in current cell and downstream cells
      # when water is not sufficient to fulfill previously (priority)
      # requirements and there are no upstream cells
      discharge <- discharge - prevWC
    }
  }
