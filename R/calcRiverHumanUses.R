...
calcRiverHumanUses <- function(humanuse, lpjml, climatetype, selectyears,
  ...
  ## endcell:         estuary cell of current cell, i.e. last cell of the river
  ##                  of which current cell is part of (exactly 1 cell)
  ...
  wat_nonag <- calcOutput("WaterUseNonAg", source = "WATERGAP2020", seasonality = "total",
                          selectyears = selectyears, climatetype = NULL, lpjml = NULL,
                          harmon_base_time = "average", aggregate = FALSE)
  ...
  CAU_magpie <- calcOutput("WaterUseCommittedAg", lpjml = lpjml, climatetype = climatetype,
  ...
  I_yearly_runoff <- collapseNames(calcOutput("YearlyRunoff",      selectyears = selectyears,
  ...
  .transformObject <- function(x) {
    ...
    object0 <- new.magpie(cells_and_regions = getCells(I_yearly_runoff),
                          years = getYears(I_yearly_runoff),
                          names = c(paste("on", getNames(I_NAg_ww), sep = "."),
                                    paste("off", getNames(I_NAg_ww), sep = ".")),
  ...
  ### Output Variables ###
  ########################
  out <- new.magpie(cells_and_regions = getCells(IO_required_wat_min),
                    years = getYears(IO_required_wat_min),
