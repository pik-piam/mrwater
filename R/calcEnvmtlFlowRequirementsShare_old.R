#' @title       calcEnvmtlFlowRequirementsShare_old
...
#' @examples
#' \dontrun{
#' calcOutput("EnvmtlFlowRequirementsShare_old", aggregate = FALSE)
#' }
#'
calcEnvmtlFlowRequirementsShare_old <- function(lpjml,
  ...
  monthly_discharge     <- setYears(calcOutput("LPJmL_new", version = lpjml["natveg"],
  ...
  if (grepl("Smakhtin", EFRmethod)) {
    ...
    if (preservationstatus == "fair") {
      ...
      stop("Please select strictness of Environmental Flow Requirements:
           Options of the preservationstatus argument are fair, good, or
           natural riverrine ecosystem condition")
    }
    ...
    if (preservationstatus != "fair") stop("The VMF method as of Pastor et al.
                                           (2014) is parametrized to a fair ecosystem preservation status.
                                           The good and natural argument can only be combined when the
                                           Smakhtin method is used.")
    ...
    EFR <- new.magpie(cells_and_regions = getCells(mean_annual_flow),
                      names = getNames(mean_monthly_flow))
