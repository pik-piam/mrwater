...
calcGrowingPeriod <- function(lpjml = c(natveg = "LPJmL4_for_MAgPIE_44ac93de", crop = "ggcmi_phase3_nchecks_9ca735cb"),
  ...
  if (stage %in% c("raw", "smoothed")) {
    ...
    yields       <- toolAggregate(yields, rel = LPJ2MAG, from = "LPJmL", to = "MAgPIE", dim = 3.1, partrel = TRUE)

    if (length(bad_crops) > 0) vcat(2, "No information on the growing period found for those crops: ",
                                    paste(unique(bad_crops), collapse = ", "))
    ...
    rm_wintercrops            <- new.magpie(getCells(sowd), getYears(sowd), names = getNames(sowd),
                                            sets = c("region", "year", "crop"), fill = 1)
