...
#' @examples
#' \dontrun{
#' calcOutput("RiverSurplusDischargeAllocation_old", aggregate = FALSE)
#' }
#'
calcRiverSurplusDischargeAllocation_old <- function(lpjml, selectyears, output, climatetype, EFRmethod, accessibilityrule, rankmethod, yieldcalib, allocationrule, thresholdtype, gainthreshold, irrigationsystem, iniyear, avlland_scen, cropmix, com_ag, multicropping) {
  ...
  required_wat_fullirrig      <- calcOutput("FullIrrigationRequirement", lpjml = lpjml,
  ...
  irrig_yieldgainpotential    <- calcOutput("IrrigYieldImprovementPotential", lpjml = lpjml, climatetype = climatetype, selectyears = selectyears, cropmix = cropmix, monetary = thresholdtype, iniyear = iniyear, yieldcalib = yieldcalib, multicropping = multicropping, aggregate = FALSE)
