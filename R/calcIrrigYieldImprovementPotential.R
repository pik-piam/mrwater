...
#' @importFrom magclass collapseNames new.magpie getYears getNames collapseDim add_columns getCells
...
calcIrrigYieldImprovementPotential <- function(climatetype, monetary, iniyear, selectyears, proxycrop, FAOyieldcalib) {
  ...
  if (FAOyieldcalib) {
    ...
    LUHcroparea   <- setYears(calcOutput("Croparea", sectoral="kcr", physical=TRUE, aggregate=FALSE)[,iniyear,], NULL)

    # calculate country FAO yields
    croplist      <- intersect(getNames(FAOproduction), getNames(LUHcroparea))
    FAOyields     <- ifelse(LUHcroparea[,,croplist]>0, FAOproduction[,,croplist] / LUHcroparea[,,croplist], 0)
    # global mean
    ...
    missingcrops  <- setdiff(getNames(LUHcroparea), getNames(FAOproduction))
