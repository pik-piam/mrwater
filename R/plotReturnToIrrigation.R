...
plotReturnToIrrigation <- function(y_axis_range, x_axis, scenario, lpjml, selectyears, climatetype, efrMethod, rankmethod, yieldcalib, allocationrule, thresholdtype, gainthreshold, irrigationsystem, landScen, cropmix, com_ag, multicropping, potential_wat = TRUE) {
  ...
  x     <- collapseNames(calcOutput("WaterPotUse",      lpjml = lpjml, gainthreshold = gainthreshold,
                                    ...
                                    allocationrule = allocationrule, thresholdtype = thresholdtype, irrigationsystem = irrigationsystem,
                                    iniyear = iniyear, landScen = landScen, cropmix = cropmix, com_ag = com_ag,
  ...
  out <- ggplot(data = df, aes_string(y = paste("IrrigArea", "on", scenario, sep = "."))) +
    ...
    theme_bw() +
    ggtitle(paste0("Return to to Irrigation with FAO-calib=", yieldcalib, " on ", landScen)) + ylab("Irrigatable Area (Mha)") + xlab("Accessible Water (km^3)")
